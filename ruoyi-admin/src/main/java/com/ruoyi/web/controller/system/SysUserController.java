...
public class SysUserController extends BaseController
{
    ...
    @Autowired
    private ISysWalletLogService sysWalletLogService;
    /**
     * 获取用户列表
     */
    ...
    public TableDataInfo list(SysUser user)
    {
        ...
        for (SysUser sysUser : list) {
            ids.add(sysUser.getUserId().toString());
        }
        ...
        if(!strings.isEmpty()){
            for (SysUser sysUser : list) {
                for (String string : strings) {
                    if(sysUser.getUserId().equals(Long.parseLong(string))){
                        sysUser.setLiveState(1);
                    }
                }
            }
        }
    ...
    public AjaxResult updateUserMoney(@RequestBody SysUser user){
        ...
        return userService.updateUserMoney(user,currentUser.getUser());
    }
    @PostMapping("/listSysWalletLog")
    public AjaxResult listSysWalletLog(@RequestBody SysWalletLog sysWalletLog){
        return sysWalletLogService.listSysWalletLog(sysWalletLog);
    }
